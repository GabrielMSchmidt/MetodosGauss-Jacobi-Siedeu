...
float **eliminacaoDeGauss(float **A, int lin) {
    ...
    for(int i = 0; i < n; i++) {
        for(int j = i + 1; j < n ; j++) {
            m = a[j][i]/a[i][i];
            cout << "Multiplicador para a linha " << j + 1 << ": " << m << endl;
            a[j][i] = 0;
            for(int k = i + 1; k < n; k++) {
                a[j][k] += -(m * a[i][k]);
            }
        ...
        cout << "Estado atual da matriz:" << endl;
        imprimirMatriz(a, n);
        cout << endl;
    }
...
void resolucaoMatrizSuperior(float **A, int lin) {
    ...
    for(int i = n - 1; i >= 0; i--) {
        s = 0;
        for(int j = i + 1 ; j < n; j++) {
            cout << "s = " << s << " + " << a[i][j] << " * " << x[j] << endl;
            s += a[i][j] * x[j];
        }
        cout << "x[" << i << "] = (" << a[i][n] << "-" << s << ")/" << a[i][i] << endl;
        x[i] = (a[i][n] - s)/a[i][i];
    }
...
int main() {
    ...
    cout << "Bem vindo ao metodo de eliminação de Gauss" << endl;
